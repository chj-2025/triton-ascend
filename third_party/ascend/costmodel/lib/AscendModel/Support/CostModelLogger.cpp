...
// Keep multi-line records and scope-boundary updates serialized.  Trace depth
// itself is thread-local so concurrent module compilations cannot corrupt one
// another's call-tree indentation.
...
int resolveLogLevelFromEnvironment() {
  ...
  if (value == "0" || value.equals_insensitive("off"))
    return 0;
  if (value == "1" || value.equals_insensitive("on") ||
      value.equals_insensitive("default"))
    return 1;
