...
static bool stageOwnsAnchor(const LogicalStage &stage,
  ...
  auto owns = [&](Operation *operation) {
    Operation *root = getTopLevelSemanticRoot(operation, splitLoopBody);
...
static Operation *getTopLevelSemanticRoot(Operation *operation,
  ...
  while (Operation *parent = root->getParentOp()) {
    ...
    // of being owned by the execute_region.  Loop-body splitting exposes
    // independent structured loops the same way: an operation directly inside
    // such a loop body resolves to itself, not to the loop shell.
    if (isFunctionLikeTTIROp(parent) ||
        parent->hasAttr("ta.auto_blockify_v1.loop") ||
        (splitLoopBody && isIndependentStructuredLoop(parent)) ||
...
collectTopLevelSemanticRoots(ModuleOp module, bool splitLoopBody) {
  ...
  auto appendOps = [&](auto &&self, Block &block, bool insideV1Loop) -> void {
    for (Operation &nested : block.getOperations()) {
      ...
      // nested recurrence/reduction work is not double-owned.  Loop-body
      // splitting applies the same shell/body rule to independent algorithmic
      // loops so their bodies are partitioned with the normal Stage rules.
      const bool splitThisLoop =
          splitLoopBody && !isV1Loop && isIndependentStructuredLoop(&nested);
      if (splitThisLoop)
        costModelLog() << "loop split: exposing body roots of " << name
                       << " " << nested.getLoc() << "\n";
      if ((!isV1Loop && !splitThisLoop) || nested.getNumRegions() == 0)
        continue;
      for (Region &region : nested.getRegions())
        for (Block &body : region)
          self(self, body, insideV1Loop || isV1Loop);
    }
  };
...
ProgramStructureAnalysis::analyze(ModuleOp module,
  ...
  structure.rootOperations =
      collectTopLevelSemanticRoots(module, splitIndependentLoopBody);
...
buildAnchorGroups(const ProgramStructure &structure,
  ...
  for (const SimtAnchorDescriptor &anchor : anchorPlan.anchors) {
    ...
    auto addPosition = [&](Operation *operation) {
      Operation *root =
          getTopLevelSemanticRoot(operation, structure.splitIndependentLoopBody);
